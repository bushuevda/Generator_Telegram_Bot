...
"""! @brief Пакет содержащий класс AnswerForwardImage."""
...
class AnswerForwardImage(Answer):
    """! Класс-наблюдатель нажатия кнопки вперед для смены изображения.
    """
    ...
    def create(self, cb_data: str, path_images: list[str],
        ...
        """! Функция создает наблюдателя за нажатием клавиши вперед для переключения изображения.
        ...
        Условия:
            1. Если self.count_image > -1 -> self.count_image -- (декремент).
            2. Если self.count_image == -1 -> self.count_image = количество изображений в списке.
        ...
        async def answer_forward_image(callback: types.CallbackQuery, bot:Bot) -> None:
            ...
            if check_user == self.manager_users.state_user[0] or check_user == self.manager_users.state_user[1]: 
                if self.count_image > -1:
                    self.count_image -= 1
                if self.count_image == -1:
                    self.count_image = len(path_images)-1
